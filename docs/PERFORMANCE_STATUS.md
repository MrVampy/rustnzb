...
## Buffer-pooling release summary

The retained body-buffer pooling change reduced the RustNZB median from
6.565 s to 5.061 s in the controlled 5 GiB raw scenario, improving throughput
from 779.89 MiB/s to 1,011.9 MiB/s. All three measured rounds passed complete
SHA-256 output validation. These figures describe one loopback workload on a
shared host and are not a general real-world performance claim.
...
- Roughly **29% of `decode_body_avx2`'s own self-time is spent in the kernel
  ...
  buffer, now backed by a real profile instead of source inspection alone.
...
buffers across articles and workers rather than making a further
decode-algorithm change. This is a hypothesis sharpened by real
...
The next optimization target is decode/dispatch/assembly overhead, guided by
the phase measurements below. No optimization is retained on a single
favorable run.

## Baseline measurements

These are three-run medians for the 5 GiB raw fixture with eight connections;
each RustNZB output was independently SHA-256 validated.

| Pipeline depth | RustNZB median | RustNZB throughput |
| ---: | ---: | ---: |
...
independently SHA-256 validates RustNZB's complete 5 GiB output; the timings
are valid for this throughput workload but do not establish post-processing
or repair performance.
...
| Buffered-line body timeout | Skipped the Tokio timeout wrapper when a complete body line was already buffered, while retaining the 20-second timeout for socket reads. | Temporary normal-path, stalled-body, and slow-but-active transfer tests passed. The controlled 5 GiB depth-2 matrix produced three valid RustNZB outputs: 6.644 s, 6.726 s, and 5.605 s (6.644 s median / 770.62 MiB/s) versus the retained 6.565 s baseline. | Reverted; the 1.2% slower median was not a repeatable gain, so the protocol timeout path remains unchanged. |

No runtime code from these rejected or incomplete experiments remains in the
current branch. None of their isolated measurements is treated as proof that
a change is safe or beneficial for RustNZB.
...
7.831 s, 5.601 s, and 7.836 s (7.831 s median), slower than its retained
6.565 s baseline; all output hashes were valid. The rewrite was reverted.
This confirms that isolated decode throughput is not sufficient when dispatch,
assembly, and scheduler contention dominate end-to-end time.
...
record three validated RustNZB runs of 6.644 s, 6.726 s, and 5.605 s
(6.644 s median, 770.62 MiB/s). Every RustNZB output matched the expected
5,368,709,120-byte SHA-256 value. Relative to the retained RustNZB depth-2
...
MiB/s per connection. Parallelism across the 8 workers looks close to linear
(no lock contention was found in the assembler, job-context map, or progress
channel), which points toward single-connection per-article CPU/syscall cost
as the more likely place to keep looking rather than cross-worker contention.
This is offered as a narrowed search area, not a conclusion, and does not by
itself explain why the SIMD decoder rewrite made the full run slower despite
being much faster in isolation.
